---
name: ste
description: Rewrites dense or ambiguous English into ASD-STE100 Simplified Technical English so a machine or non-native reader cannot misparse it. Use when text must be parsed without a human to resolve ambiguity — tool descriptions, error messages, inter-agent instructions, system prompts, status reports — or when asked to disambiguate, apply STE100, or do a plain-language rewrite; not for creative or marketing copy.
user-invocable: true
